...
async def websocket_chat_endpoint(
    ...
    """
    # Authenticate user from token, API key, or query params
    user = None
    auth_error = None
    
    try:
        # Try to get API key from headers or query params
        ...
        if api_key and verify_api_key(api_key):
            # Use API key authentication - create a user object
...
async def handle_text_message(
    ...
    # Get agent loop from app state (set during startup)
    from apps.api.main import agent_loop
                ...
                if data.get("status") == "start":
                    ...
                    async with get_async_session() as db:
                        await record_tool_end(
                            db,
                            run_id=tool_run.id if tool_run else 1,  # Get from start event in real implementation
                            status=data.get("status"),
...
async def unified_chat_endpoint(
    ...
    if request.text_input and request.text_input.strip():
        ...
        raise HTTPException(
            status_code=400,
            detail="Either text_input or audio_b64 must be provided"
        )
