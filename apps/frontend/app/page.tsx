...
export default function Home() {
  ...
  const [tokenTimeline, setTokenTimeline] = useState<TokenTimelineResponse | null>(null);
  ...
  const loadDashboard = async () => {
    try {
      ...
      const [
        ...
        getTokenTimeline({ days: DEFAULT_WINDOW_DAYS, interval: "day" }),
  ...
  const recentTokenEntries = useMemo(
    () => (tokenTimeline?.data ?? []).slice(-6).reverse(),
    [tokenTimeline]
  );
        ...
        <>
          ...
          <section className="card">
            <div className="card-header">
              <div>
                <h2 className="card-title">Token usage timeline</h2>
                <p className="card-subtitle">
                  Daily breakdown of tokens consumed and generated by the assistant.
                </p>
              </div>
              <span className="chip">
                {recentTokenEntries.length} of {tokenTimeline?.data.length ?? 0} datapoints
              </span>
            </div>
            <div className="timeline-list">
              {recentTokenEntries.length === 0 && (
                <div className="empty-state">No token activity recorded for this period.</div>
              )}
              {recentTokenEntries.map((entry) => (
                <div key={entry.period} className="timeline-item">
                  <div>
                    <h3>{formatDate(entry.period)}</h3>
                    <p className="muted-text">
                      {formatNumber(entry.message_count)} messages ·{" "}
                      {formatNumber(entry.total_tokens)} tokens
                    </p>
                  </div>
                  <div className="timeline-values">
                    <span className="badge badge-muted">
                      In&nbsp;{formatNumber(entry.tokens_in)}
                    </span>
                    <span className="badge badge-info">
                      Out&nbsp;{formatNumber(entry.tokens_out)}
                    </span>
                  </div>
                </div>
              ))}
