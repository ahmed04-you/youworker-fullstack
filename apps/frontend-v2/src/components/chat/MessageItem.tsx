...
const MessageAvatar = memo(function MessageAvatar({ role }: { role: Message['role'] }) {
  const isUser = role === 'user'

  return (
    <div
      className={`shrink-0 w-8 h-8 rounded-full flex items-center justify-center ${
        isUser ? 'bg-gradient-brand' : 'bg-gradient-slate'
      }`}
      role="img"
  ...
  function MessageItem({ message }: MessageItemProps) {
    ...
    const markdownComponents = useMemo<Partial<Components>>(
      () => ({
        ...
        h4: ({ children }) => (
          <h4 className="text-base font-semibold mb-2 mt-2 text-white">{children}</h4>
        ),
        h5: ({ children }) => (
          <h5 className="text-sm font-semibold mb-1 mt-2 text-white">{children}</h5>
        ),
        h6: ({ children }) => (
          <h6 className="text-sm font-semibold mb-1 mt-2 text-white/90">{children}</h6>
        ),
        // Paragraphs
        ...
        tr: ({ children }) => (
          <tr className="border-b border-[var(--color-glass-dark)] last:border-0">{children}</tr>
        ),
        th: ({ children }) => (
    ...
    return (
      <div className={`flex gap-3 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
        ...
        {/* Message content */}
        <div
          className={`flex-1 max-w-[70%] ${isUser ? 'items-end' : 'items-start'} flex flex-col gap-1`}
        >
          ...
          <span className="text-xs text-white/40 px-2">{formatTimestamp(message.timestamp)}</span>
        </div>
      </div>
    )
  },
