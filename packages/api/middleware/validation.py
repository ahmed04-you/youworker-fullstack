...
class ValidationMiddleware:
    ...
    def validate_ingest(request_data: Dict[str, Any]) -> IngestRequest:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Invalid ingestion request: {str(e)}"
            )
    ...
    def validate_tool_call(request_data: Dict[str, Any]) -> ToolCallRequest:
        ...
        try:
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Invalid tool call: {str(e)}"
            )
...
def validate_batch_request(request_data: Dict[str, Any], max_items: int = 100) -> List[Dict[str, Any]]:
    """
    ...
    if 'items' not in request_data:
        raise HTTPException(
            status_code=400,
            detail="Batch request must contain 'items' field"
        )
